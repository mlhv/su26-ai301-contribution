...
**PR Link:** [#2863](https://github.com/open-telemetry/opentelemetry-ebpf-instrumentation/pull/2863)
...
**Maintainer Feedback:**
- **MrAlias** (reviewer) left a summary review — *"The replacement is not yet behaviorally equivalent to the spans it suppresses: signal gating, resource policy, and HTTP exporter behavior can all diverge from the corresponding trace pipeline"* — plus three inline comments, all fixed and pushed:
  - `logs.go:110` — `suppressQueueProcessingSpans` was computed once globally, so a service scoped to `exports: [traces]` had its sub-spans suppressed even though the logs receiver (gated on `ExportModes.CanExportLogs()`) never sent it the replacement log — the timing data disappeared entirely. Fixed in [`3b6ba814`](https://github.com/mlhv/opentelemetry-ebpf-instrumentation/commit/3b6ba814b635924a8b296ea63c86d6eeeef23357): suppression is now computed per span-group, gated on the same `CanExportLogs()` check.
  - `logsgen.go:42` — `GenerateLogs` never applied `attributes.select` resource filtering or `ctxInfo.ExtraResourceAttributes`, unlike the trace pipeline. Fixed in [`3b3aa58f`](https://github.com/mlhv/opentelemetry-ebpf-instrumentation/commit/3b3aa58f61b9d863eb73845ab916dbbd3a0389fe); a write-order bug in that same fix (extraResAttrs must win on key collision, matching `tracesgen`) was caught in self-review and fixed in [`e03148c6`](https://github.com/mlhv/opentelemetry-ebpf-instrumentation/commit/e03148c61b442db875bd36317b3513cf6061c008), which also exported `tracesgen.AddAttrsToMap` so both paths share one implementation instead of two that can drift.
  - `logs.go:219` — the HTTP logs exporter double-buffered queue/retry: `otlphttpexporter`'s factory already builds its own internal queue+retry from `config.QueueConfig`/`RetryConfig`, and the outer `exporterhelper.NewLogs` wrap applied the same real settings again on top. Fixed in [`7accb1ca`](https://github.com/mlhv/opentelemetry-ebpf-instrumentation/commit/7accb1ca88942033f3a66f375fc26e32f0ded4a6): extracted `buildHTTPLogsExporterConfig`, mirroring `getTracesExporter`'s disable-inner/wrap-outer-once pattern.
- **marctc** (Approver) raised a scope concern: *"it's adding the whole log pipeline... I'd [prefer] to [do this] in separate PRs."* MrAlias agreed, adding a concrete requirement: *"The plumbing PR should include an end-to-end OTLP logs export test; the current debug-exporter test only covers generation and wiring."*
- Agreed to split this PR into two along the plumbing/feature boundary. This PR now holds the queue/processing feature only (`logsgen`, the suppression toggle, the receiver/wiring); the generic logs-export plumbing (`LogsConfig`, `getLogsExporter`, config wiring) was extracted into a new PR — [#2969](https://github.com/open-telemetry/opentelemetry-ebpf-instrumentation/pull/2969) — which this PR will depend on. #2969 adds `TestLogsExportDelivery`, a new test proving real OTLP delivery over HTTP and gRPC to an in-process receiver (decoding the actual wire payload), addressing MrAlias's ask.

**Status:** Open — split in progress. #2969 (plumbing) opened and awaiting review; once it merges, this PR will be rebased down to the queue/processing-only diff, then reopened/updated accordingly. Branch `queue-processing-spans-to-logs` kept around (not deleted) until both land.
